...
import formatNumber, {addSignPrefixAndSuffix} from './formatNumber';
...
export default React.forwardRef<TextInput, CurrencyInputProps>((props, ref) => {
    ...
    const handleChangeText = React.useCallback(
        (text: string) => {
            ...
            const textNumericValue = textWithoutPrefixAndSufix.replace(
                /\D+/g,
                '',
            );

            const numberValue =
                Number(textNumericValue) * (isNegativeValue ? -1 : 1);
            ...
            if (
                !textNumericValue ||
                (!numberValue && zerosOnValue === precision)
            ) {
        ...
        <TextInput
            ...
            selection={
                suffix
                    ? {
                          start: Math.max(
                              textInputValue.length - suffix.length,
                              0,
                          ),
                      }
                    : props?.selection
