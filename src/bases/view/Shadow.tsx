...
export function R(value: number): number {
    ...
    if (isWeb) return Math.floor(value);
...
export const Shadow: React.FC<ShadowProps> = ({
    ...
    const radii: CornerRadius = useMemo(() => {
        ...
        const cornerRadiusPartial: Partial<CornerRadius> = (() => {
            if (radiusProp) {
                if (typeof radiusProp === 'number')
                    ...
                    return objFromKeys(
                        cornersArray,
                        k => radiusProp[k] ?? radiusProp.default,
                    );
            }
            ...
            let mergedStyle: Record<Corner, number | undefined> = {
            ...
            if (getViewStyleRadius) {
                ...
                mergedStyle = objFromKeys(
                    cornersArray,
                    k =>
                        mergedViewStyle[cornerToStyle(k, false)] ??
                        mergedViewStyle[cornerToStyle(k, true)] ??
                        mergedViewStyle?.borderRadius,
                ) as Record<Corner, number | undefined>;
            }

            // Only enter block if there is a undefined corner that may now be defined;
            if (
                getChildRadius &&
                Object.values(mergedStyle).includes(undefined)
            ) {
                if (React.Children.count(children) > 1)
                    throw new Error(
                        'Only single child is accepted in Shadow component with getChildRadius={true} (default value). You should wrap it in a View or change this property to false and manually enter the borderRadius in the radius property.',
                    );
                /** May be an array of styles. */
                const childStyleTemp: ViewStyle | undefined = (
                    React.Children.only(children) as JSX.Element | undefined
                )?.props?.style;
                ...
                mergedStyle = objFromKeys(
                    ...
                    k =>
                        ...
                        childStyle[cornerToStyle(k, false)] ??
                        childStyle[cornerToStyle(k, true)] ??
        ...
        const radiiPreSizeLimit = objFromKeys(cornersArray, k =>
            R(Math.max(cornerRadiusPartial[k] ?? 0, 0)),
        );
        ...
        if (typeof width === 'number' && typeof height === 'number') {
            ...
            const minRatio = Math.min(
                ...
                width /
                    (radiiPreSizeLimit.topLeft + radiiPreSizeLimit.topRight), // top
                height /
                    (radiiPreSizeLimit.topRight +
                        radiiPreSizeLimit.bottomRight), // right
                width /
                    (radiiPreSizeLimit.bottomLeft +
                        radiiPreSizeLimit.bottomRight), // bottom
                height /
                    (radiiPreSizeLimit.topLeft + radiiPreSizeLimit.bottomLeft), // left
            );
            if (minRatio < 1)
                result = objFromKeys(cornersArray, k =>
                    R(radiiPreSizeLimit[k] * minRatio),
                );
        }
    ...
    const shadow = useMemo(() => {
        ...
        const startColor =
            startColorProp === 'transparent' ? '#0000' : startColorProp;
        const finalColor =
            finalColorProp === 'transparent' ? '#0000' : finalColorProp;

        const startColorRgb = parseToRgb(startColor) as Omit<
            RgbaColor,
            'alpha'
        > & {alpha?: number};
        const finalColorRgb = parseToRgb(finalColor) as Omit<
            RgbaColor,
            'alpha'
        > & {alpha?: number};
        ...
        const startColorWoOpacity = rgbToColorString({
            ...startColorRgb,
            alpha: undefined,
        }); // overwrite alpha
        const finalColorWoOpacity = rgbToColorString({
            ...finalColorRgb,
            alpha: undefined,
        });
        ...
        const activeSides: Record<Side, boolean> = objFromKeys(sidesArray, k =>
            sidesProp.includes(k),
        );

        /** Which corners will have shadow. */
        const activeCorners: Record<Corner, boolean> = objFromKeys(
            cornersArray,
            k => cornersProp.includes(k),
        );
        ...
        const linearGradient = [
            ...
            <Stop
                offset={1}
                stopColor={finalColorWoOpacity}
                stopOpacity={finalColorOpacity}
                key="2"
            />,
        ];
                ...
                {activeSides.bottom && (
                    <Svg
                        ...
                        <Rect
                            ...
                            height={distance}
                            fill="url(#bottom)"
                            x={-sumDps(bottomLeft, bottomRight)}
                        />
                    </Svg>
                )}
                {activeSides.top && (
        ...
        The starting point is the clockwise external arc init point. */}
                ...
                {activeCorners.topLeft && (
                    <Svg
                        width={topLeftShadow + additional}
                        height={topLeftShadow + additional}
                        style={{
                            position: 'absolute',
                            top: -distance,
                            left: -distance,
                        ...
                        <Path
                            fill="url(#topLeft)"
                            d={`M0,${topLeftShadow} a${topLeftShadow},${topLeftShadow} 0 0 1 ${topLeftShadow} ${-topLeftShadow} v${distance} ${
                                paintInside
                ...
                {activeCorners.topRight && (
                    <Svg
                        width={topRightShadow + additional}
                        height={topRightShadow + additional}
                        style={{
                            position: 'absolute',
                            top: -distance,
                            left: width,
                        ...
                        <Path
                            fill="url(#topRight)"
                            d={`M0,0 a${topRightShadow},${topRightShadow} 0 0 1 ${topRightShadow},${topRightShadow} h${-distance} ${
                                paintInside
                ...
                {activeCorners.bottomLeft && (
                    <Svg
                        width={bottomLeftShadow + additional}
                        height={bottomLeftShadow + additional}
                        style={{
                            position: 'absolute',
                            top: height,
                            left: -distance,
                        ...
                        <Path
                            fill="url(#bottomLeft)"
                            d={`M${bottomLeftShadow},${bottomLeftShadow} a${bottomLeftShadow},${bottomLeftShadow} 0 0 1 ${-bottomLeftShadow},${-bottomLeftShadow} h${distance} ${
                                paintInside
                ...
                {activeCorners.bottomRight && (
                    <Svg
                        width={bottomRightShadow + additional}
                        height={bottomRightShadow + additional}
                        style={{
                            position: 'absolute',
                            top: height,
                            left: width,
                        ...
                        <Path
                            ...
                            d={`M${bottomRightShadow},0 a${bottomRightShadow},${bottomRightShadow} 0 0 1 ${-bottomRightShadow},${bottomRightShadow} v${-distance} ${
                                paintInside
                        ...
                        <Defs>
                            <Mask id="maskPaintBelow">
                                ...
                                <Rect
                                    ...
                                    height={topRight}
                                    x={width}
                                    transform={`translate(${-topRight}, 0)`}
                                    fill="#000"
                                />
                                <Rect
                                    width={bottomLeft}
                                    height={bottomLeft}
                                    y={height}
                                    transform={`translate(0, ${-bottomLeft})`}
                                    fill="#000"
                                />
                                <Rect
                                    width={bottomRight}
                                    ...
                                    x={width}
                                    y={height}
                                    transform={`translate(${-bottomRight}, ${-bottomRight})`}
                                    fill="#000"
                                />
                            </Mask>
                        </Defs>
